...
class MovimentoDao extends Model{
    public function saldoEstoque($id_produto){
        $sql = "SELECT saldoestoque as saldo FROM movimento 
        WHERE id_produto = $id_produto ORDER BY id_movimento DESC LIMIT 1";
    ...
    public function lista($limit = null){
        ...
        m.id_produto = p.id_produto AND m.id_tipo_movimento = t.id_tipo_movimento";
        if($limit){
          $sql .=  " LIMIT $limit";
        }
